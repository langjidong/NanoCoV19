...
#  This script should be executed from unitigging/Alpha.canu.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./Alpha.canu.ovlStore.BUILDING/$jobname.info -a ! -e ./Alpha.canu.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './Alpha.canu.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./Alpha.canu.ovlStore.BUILDING \
  -S ../Alpha.canu.seqStore \
  -C  ./Alpha.canu.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
